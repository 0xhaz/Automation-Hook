...
contract Automate is Ownable, AutoMateEIP712, IAutoMate {
    /// @dev The PoolKey is a structure that uniquely identifies a liquidity pool by
    /// storing its details -- the two currencies involved, the swap fee, tick spacing
    /// and hooks of the pool
    ...
    Task[] private _tasks;
    /// @dev This is to save gas from needing to loop through expired tasks to get
    /// the next active task index, since there could be expired tasks in the array
    uint256 private _activeTaskStartingIdx;
    uint256 private _taskIdCounter;
    address private _hookAddress;
    uint16 private _protocolFeeBP;
    /// @dev 1% decay on jitBounty per minute based on how close the execution time is to scheduled time
    uint16 private _bountyDecayBPPerMinute;
    address private _feeReceiver;

    modifier onlyFromHook() {
        if (msg.sender != _hookAddress) revert OnlyFromAuthorizedHook();
        _;
    }
    ...
    function executeTask(bytes calldata hookData) external payable onlyFromHook {
        ...
        _tasks.pop();
        // save gas to skip the loop filter expired tasks
    ...
    function _tryGetClosestToJITIdx(uint256 startingIdx)
        internal
        view
        returns (uint256 closestToJITIdx, uint256 activeStartingIdx)
    {
    ...
    function _executeTaskBasedOnTaskType(Task memory task) internal {
        ...
        if (task.taskType == TaskType.ERC20_TRANSFER || task.taskType == TaskType.CONTRACT_CALL_WITH_ERC20) {
            // Approve before allowing the callingAddress to spend the token
        ...
        internal
    {
        ...
        uint256 finalBounty = minsBeforeJIT >= _FULLY_DECAYED_IN_MINUTES
            ? 0
